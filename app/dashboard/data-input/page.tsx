...
import {
    ...
    FileText, Clock, Wifi, WifiOff, Send,
    CheckCircle2, AlertTriangle, RefreshCw, Pencil,
} from "lucide-react";
import { Status } from "@/type";

interface SensorEntry {
    id: number; temperature: number; humidity: number; vibration: number;
    deviceId: string; location: string; notes: string; timestamp: string;
    source: "esp32" | "manual"; status: Status; saved: boolean;
}
...
function ValueGauge({ value, min, max, unit, color, label }: {
    value: number; min: number; max: number; unit: string; color: string; label: string;
}) {
...
const EMPTY_FORM = {
    ...
    deviceId: "ESP32-001", location: "Server Room A",
    notes: "", timestamp: "", useNow: true as boolean,
};
...
export default function DataInputPage() {
    ...
    useEffect(() => {
        ...
        const id = setInterval(() => {
            ...
            setForm(f => ({ ...f, temperature: String(data.temperature), humidity: String(data.humidity), vibration: String(data.vibration) }));
        }, 5000);
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        const status = deriveStatus(+form.temperature, +form.humidity, +form.vibration);
        const id = nextId;

        // Optimistically add to local list
        const entry: SensorEntry = {
            id, temperature: +form.temperature, humidity: +form.humidity, vibration: +form.vibration,
            deviceId: form.deviceId, location: form.location, notes: form.notes, timestamp: ts,
            source: autoRead ? "esp32" : "manual", status, saved: false,
        };
        ...
        try {
            ...
            const { error } = await supabase.from("sensor_readings").insert({
                ...
                ...(form.useNow ? {} : { created_at: new Date(form.timestamp).toISOString() }),
            });

            if (error) throw error;

            // Mark as saved
            setEntries(prev => prev.map(r => r.id === id ? { ...r, saved: true } : r));
            setSubmitted(true);
            ...
            setEntries(prev => prev.map(r => r.id === id ? { ...r, saved: false } : r));
        } finally {
            setSaving(false);
        }
    }

    function handleReset() { setForm(EMPTY_FORM); setErrors({}); setSaveError(null); }

    const inputCls = (key: string) =>
        `w-full bg-background border rounded-xl px-4 py-2.5 text-sm text-foreground outline-none transition-all font-mono placeholder:text-muted-foreground/50 focus:ring-2 focus:ring-ring/30 ${errors[key] ? "border-red-400 focus:border-red-400" : "border-border focus:border-ring"}`;

    return (
        <div className="flex flex-col gap-6 p-4 md:p-6 bg-background min-h-screen">
            <div>
                <p className="text-xs font-bold text-muted-foreground font-mono uppercase tracking-widest mb-1">Sensor Input</p>
                <h1 className="text-3xl font-bold text-foreground tracking-tight">Data Input</h1>
                <p className="text-sm text-muted-foreground mt-1">Manual entry or pull directly from your ESP32 device</p>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="xl:col-span-2 flex flex-col gap-5">
                    ...
                    {/* ESP32 bar */}
                    <div className="flex items-center justify-between gap-4 bg-card border border-border rounded-2xl px-5 py-4 flex-wrap">
                        <div className="flex items-center gap-3">
                            <div className={`w-9 h-9 rounded-xl flex items-center justify-center border ${esp32On ? "bg-emerald-50 border-emerald-200 dark:bg-emerald-500/10 dark:border-emerald-500/20" : "bg-muted border-border"}`}>
                                {esp32On ? <Wifi className="w-4 h-4 text-emerald-500" /> : <WifiOff className="w-4 h-4 text-muted-foreground" />}
                            </div>
                            <div>
                                <p className="text-sm font-semibold text-foreground">ESP32 {esp32On ? "Connected" : "Disconnected"}</p>
                                <p className="text-xs text-muted-foreground font-mono">{esp32On ? form.deviceId + " · " + form.location : "No device found"}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-2 flex-wrap">
                            <label className="flex items-center gap-2 cursor-pointer select-none">
                                <span className="text-xs text-muted-foreground font-medium">Auto-read</span>
                                <div onClick={() => esp32On && setAutoRead(a => !a)}
                            ...
                            <button type="button" onClick={handleFetchNow} disabled={!esp32On || reading}
                            ...
                            <button type="button" onClick={() => setEsp32On(v => !v)}
                    ...
                    <form onSubmit={handleSubmit} className="bg-card border border-border rounded-2xl p-5 shadow-sm flex flex-col gap-5">
                        ...
                        {/* Save error */}
                        {saveError && (
                            <div className="flex items-start gap-3 bg-red-50 border border-red-200 dark:bg-red-500/10 dark:border-red-500/20 rounded-xl px-4 py-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <Field label="Temperature" icon={<Thermometer className="w-3.5 h-3.5 text-orange-500" />} hint="Range: −50 to 100°C">
                                <div className="relative">
                                    <input type="number" step="0.1" min="-50" max="100" value={form.temperature}
                                        onChange={e => handleChange("temperature", e.target.value)} placeholder="28.4" className={inputCls("temperature")} />
                            ...
                            <Field label="Humidity" icon={<Droplets className="w-3.5 h-3.5 text-sky-500" />} hint="Range: 0 to 100%">
                                <div className="relative">
                                    <input type="number" step="0.1" min="0" max="100" value={form.humidity}
                                        onChange={e => handleChange("humidity", e.target.value)} placeholder="62.1" className={inputCls("humidity")} />
                            ...
                            <Field label="Vibration" icon={<Activity className="w-3.5 h-3.5 text-violet-500" />} hint="Range: 0 to 10g">
                                <div className="relative">
                                    <input type="number" step="0.001" min="0" max="10" value={form.vibration}
                                        onChange={e => handleChange("vibration", e.target.value)} placeholder="0.03" className={inputCls("vibration")} />
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <Field label="Device ID" icon={<Cpu className="w-3.5 h-3.5 text-muted-foreground" />}>
                                <select value={form.deviceId} onChange={e => handleChange("deviceId", e.target.value)} className={inputCls("deviceId")}>
                                    <option>ESP32-001</option><option>ESP32-002</option><option>ESP32-003</option>
                                </select>
                            </Field>
                            <Field label="Location" icon={<MapPin className="w-3.5 h-3.5 text-muted-foreground" />}>
                                <select value={form.location} onChange={e => handleChange("location", e.target.value)} className={inputCls("location")}>
                                    <option>Server Room A</option><option>Server Room B</option>
                                    <option>Lab Floor 2</option><option>Outdoor Station</option>
                                </select>
                            </Field>
                        </div>
                        ...
                        <Field label="Timestamp" icon={<Clock className="w-3.5 h-3.5 text-muted-foreground" />}>
                            <div className="flex items-center gap-3">
                                <label className="flex items-center gap-2 cursor-pointer select-none">
                                    <div onClick={() => setForm(f => ({ ...f, useNow: !f.useNow }))}
                        ...
                        <Field label="Notes" icon={<FileText className="w-3.5 h-3.5 text-muted-foreground" />} hint="Optional — describe the reading context">
                            <textarea value={form.notes} onChange={e => handleChange("notes", e.target.value)}
                                ...
                                rows={3} className={`${inputCls("notes")} resize-none font-sans`} />
                        </Field>

                        {/* Actions */}
                        <div className="flex items-center gap-3 pt-1 border-t border-border">
                            <button type="submit" disabled={saving}
                                className="flex items-center gap-2 text-sm font-bold px-5 py-2.5 rounded-xl text-white shadow-md hover:opacity-90 active:scale-95 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
                                style={{ background: "#1F263E" }}>
                                {saving ? <><RefreshCw className="w-4 h-4 animate-spin" />Saving…</>
                                    : submitted ? <><CheckCircle2 className="w-4 h-4 text-emerald-400" />Saved!</>
                            ...
                            <span className="ml-auto text-xs text-muted-foreground font-mono">
                                {entries.filter(e => e.saved).length} saved · {entries.length} this session
                            </span>
                        </div>
                    </form>
                </div>
                    ...
                    <div className="bg-card border border-border rounded-2xl p-5 shadow-sm sticky top-6">
                        ...
                        {hasValues ? (
                            <div className="flex flex-col gap-5">
                                ...
                                <ValueGauge value={previewVib} min={0} max={0.5} unit="g"
                                    color={previewVib > 0.18 ? "text-red-500" : previewVib > 0.09 ? "text-amber-500" : "text-violet-500"}
                                    label="Vibration" />
                                ...
                                {previewStatus !== "normal" && (
                                    <div className={`flex items-start gap-2 rounded-xl px-3 py-2.5 border text-xs ${previewStatus === "critical" ? "bg-red-50 border-red-200 text-red-700 dark:bg-red-500/10 dark:border-red-500/20 dark:text-red-400" : "bg-amber-50 border-amber-200 text-amber-700 dark:bg-amber-500/10 dark:border-amber-500/20 dark:text-amber-400"}`}>
                                        <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                                        <span>{previewStatus === "critical" ? "One or more values exceed critical thresholds." : "One or more values are above warning thresholds."}</span>
                                    </div>
                                )}
                            </div>
                        ) : (
                            <div className="flex flex-col items-center justify-center py-10 text-center gap-3">
                                ...
                                <p className="text-sm text-muted-foreground">Start entering values to see a live preview</p>
                            </div>
                        )}
                    </div>
            ...
            {entries.length > 0 && (
                <div>
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">This Session</span>
                        <span className="text-xs text-muted-foreground font-mono ml-1">{entries.length} {entries.length === 1 ? "entry" : "entries"}</span>
                    </div>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-border bg-muted/40">
                                        {["#", "Time", "Temp", "Humidity", "Vibration", "Device", "Location", "Source", "Status", "Saved", "Notes"].map(h => (
                                            <th key={h} className="px-4 py-3 text-left text-xs font-bold text-muted-foreground uppercase tracking-widest whitespace-nowrap first:pl-5">{h}</th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody>
                                    {entries.map((r, i) => (
                                        <tr key={r.id} className={`border-b border-border hover:bg-primary/5 transition-colors ${i % 2 === 0 ? "bg-card" : "bg-muted/20"}`}>
                                            ...
                                            <td className="px-4 py-3 whitespace-nowrap font-mono font-bold text-base text-orange-500">{r.temperature}<span className="text-sm font-normal text-orange-300">°C</span></td>
                                            <td className="px-4 py-3 whitespace-nowrap font-mono font-bold text-base text-sky-500">{r.humidity}<span className="text-sm font-normal text-sky-300">%</span></td>
                                            <td className="px-4 py-3 whitespace-nowrap font-mono font-bold text-base text-violet-500">{r.vibration}<span className="text-sm font-normal text-violet-300">g</span></td>
                                            <td className="px-4 py-3 whitespace-nowrap">
                                            ...
                                            <td className="px-4 py-3 whitespace-nowrap">
                                                <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full border ${r.source === "esp32" ? "bg-sky-50 text-sky-700 border-sky-200 dark:bg-sky-500/10 dark:text-sky-400 dark:border-sky-500/20" : "bg-muted text-muted-foreground border-border"}`}>

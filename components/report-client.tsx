...
import {
    ...
    TrendingUp, TrendingDown, CheckCircle2, Zap, Download, Cpu,
} from "lucide-react";
import {
...
import type { DailySummary } from "@/lib/supabase/sensor-queries";

// ─── Props ────────────────────────────────────────────────────────────────────
interface Props {
    dailySummaries: DailySummary[];
    weeklyTrend: { date: string; temperature: number; humidity: number; vibration: number; alerts: number }[];
    weekAvgTemp: number;
    weekAvgHum: number;
    weekPeakVib: number;
    totalAlerts: number;
...
function TypeIcon({ type }: { type: Props["alertLog"][0]["type"] }) {
        ...
        <div className="relative overflow-hidden bg-card border border-border rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="pl-3 flex flex-col gap-3">
                <div className="flex items-start justify-between">
                    <div className="w-10 h-10 rounded-2xl bg-muted border border-border flex items-center justify-center shrink-0">{icon}</div>
                    {deltaLabel && (
...
export function ReportClient({
    dailySummaries, weeklyTrend, weekAvgTemp, weekAvgHum,
    weekPeakVib, totalAlerts, totalReadings, alertLog,
}: Props) {

    const vibData = dailySummaries.map(d => ({
        date: d.date, peak: d.peakVibration, avg: d.avgVibration,
    }));

    const dateRange = dailySummaries.length >= 2
        ? `${dailySummaries[0].date} – ${dailySummaries[dailySummaries.length - 1].date}`
        : dailySummaries[0]?.date ?? "No data";
        ...
        <div className="flex flex-col gap-8 p-4 md:p-6 bg-background min-h-screen">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div>
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-foreground tracking-tight">Analytics Report</h1>
                    <p className="text-sm text-muted-foreground mt-1">
                        <span className="font-semibold text-foreground">{totalReadings.toLocaleString()}</span> readings ·{" "}
                        <span className="font-semibold text-foreground">{dailySummaries.length}</span> days
                    </p>
                </div>
                <button className="flex items-center gap-2 text-sm font-semibold px-5 py-2.5 rounded-xl text-white shadow-md transition-all hover:opacity-90 active:scale-95 self-start sm:self-auto"
            ...
            {totalReadings === 0 ? (
                <div className="flex items-center justify-center py-20 text-muted-foreground font-mono text-sm">
                    No data in the last 7 days — add some readings to see the report
                </div>
            ) : (
                <>
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                        <KpiCard label="Avg Temperature" value={`${weekAvgTemp}°C`} sub="7-day average"
                            delta="up" deltaLabel="Past 7 days" accent="from-orange-400 to-amber-400"
                            icon={<Thermometer className="w-4 h-4 text-orange-500" />} />
                        <KpiCard label="Avg Humidity" value={`${weekAvgHum}%`} sub="7-day average"
                            delta="neutral" deltaLabel="Stable" accent="from-sky-400 to-cyan-400"
                            icon={<Droplets className="w-4 h-4 text-sky-500" />} />
                        <KpiCard label="Peak Vibration" value={`${weekPeakVib}g`} sub="Highest recorded"
                            delta={weekPeakVib > 0.18 ? "up" : "neutral"} deltaLabel={weekPeakVib > 0.18 ? "Critical" : "Normal"}
                            accent="from-violet-400 to-purple-400"
                            icon={<Activity className="w-4 h-4 text-violet-500" />} />
                        <KpiCard label="Alert Events" value={String(totalAlerts)} sub="Threshold crossings"
                            delta="neutral" deltaLabel="All resolved" accent="from-amber-400 to-orange-400"
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                            <ChartCard title="Temperature" sub="Daily average · °C"
                            ...
                            <ChartCard title="Humidity" sub="Daily average · %"
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                            <div className="lg:col-span-2">
                                <ChartCard title="Vibration — Peak vs Average" sub="Daily readings · g"
                                    accentClass="bg-gradient-to-r from-violet-400 to-purple-500"
                                    badge={<span className="text-xs font-mono text-violet-500 bg-violet-50 border border-violet-100 dark:bg-violet-500/10 dark:border-violet-500/20 px-2.5 py-1 rounded-full font-semibold flex items-center gap-1"><Zap className="w-3 h-3" />Sensor</span>}>
                                    <ResponsiveContainer width="100%" height={240}>
                                        <ComposedChart data={vibData} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
                                            ...
                                            <Line type="monotone" dataKey="avg" name="Avg vib (g)" stroke="#7c3aed" strokeWidth={2.5}
                                                dot={{ r: 4, fill: "#7c3aed", strokeWidth: 0 }} activeDot={{ r: 6, stroke: "#fff", strokeWidth: 2 }} />
                                        </ComposedChart>
                                    </ResponsiveContainer>
                                </ChartCard>
                            </div>
                            <div className="lg:col-span-1">
                                <ChartCard title="Daily Alerts" sub="Events per day" accentClass="bg-gradient-to-r from-amber-400 to-orange-400">
                                    <ResponsiveContainer width="100%" height={240}>
                                    ...
                                    <TableBody>
                                        {[...dailySummaries].reverse().map((d, i) => (
                                            <TableRow key={d.date} className={`border-border hover:bg-primary/5 transition-colors ${i % 2 === 0 ? "bg-card" : "bg-muted/20"}`}>
                                                ...
                                                <TableCell><span className="font-mono text-base font-bold text-orange-500">{d.avgTemp}°C</span></TableCell>
                                                ...
                                                <TableCell><span className="font-mono text-base font-bold text-sky-500">{d.avgHumidity}%</span></TableCell>
                                                <TableCell className="whitespace-nowrap">
                                                ...
                                                <TableCell><span className="font-mono text-sm font-semibold text-foreground">{d.totalReadings}</span></TableCell>
                                            </TableRow>
                                        ))}
                                    </TableBody>
                                </Table>
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center gap-2">
                                    <div className="w-1 h-4 rounded-full bg-gradient-to-b from-red-400 to-amber-400" />
                                    <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Alert Log</span>
                                </div>
                                <span className="text-xs font-mono text-muted-foreground">{alertLog.length} events · all resolved</span>
                            </div>
                            <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
                                        ...
                                        <TableBody>
                                            {alertLog.map((a, i) => (
                                                <TableRow key={a.id} className={`border-border hover:bg-primary/5 transition-colors ${i % 2 === 0 ? "bg-card" : "bg-muted/20"}`}>
                                                    ...
                                                    <TableCell className="px-4 whitespace-nowrap">
                                                        <span className="flex items-center gap-2 text-sm font-semibold text-foreground capitalize"><TypeIcon type={a.type} />{a.type}</span>
                                                    </TableCell>

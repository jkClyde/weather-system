...
function StatCard({ s }: { s: Props["stats"][0] }) {
                        ...
                        <div className="relative bg-card border border-border rounded-2xl p-5 shadow-sm overflow-hidden">
                            ...
                            <div className="flex items-center justify-between gap-3 mb-5">
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground font-mono mt-0.5">Today — 30 min intervals</p>
                                </div>
                            ...
                            {chartData.length === 0 ? (
                                ...
                                <ResponsiveContainer width="100%" height={260}>
                                    <AreaChart data={chartData} margin={{ top: 4, right: 4, left: -16, bottom: 0 }}>
                                        ...
                                        <XAxis dataKey="time" tick={{ fontSize: 11, fill: "var(--muted-foreground)", fontFamily: "monospace" }} tickLine={false} axisLine={false} />
                                        <YAxis tick={{ fontSize: 11, fill: "var(--muted-foreground)", fontFamily: "monospace" }} tickLine={false} axisLine={false} />
                                        <Tooltip content={<CustomTooltip />} />
                                        <Legend wrapperStyle={{ fontSize: 12, paddingTop: 12 }} iconType="circle" iconSize={8}
                                            formatter={(v, e) => <span style={{ color: (e as any).color, fontWeight: 600, fontSize: 12 }}>{v}</span>} />
                                ...
                                {recentReadings.length === 0 ? (
                                    ...
                                    recentReadings.map((r, i) => (
                                        <TableRow key={r.id} className={`border-border hover:bg-primary/5 transition-colors ${i % 2 === 0 ? "bg-card" : "bg-muted/20"}`}>
                                            ...
                                            <TableCell className="whitespace-nowrap">
                                                <span className="font-mono text-base font-bold text-violet-500">{r.vibration}<span className="text-sm font-normal text-violet-300">g</span></span>
                                            </TableCell>
                                            <TableCell className="whitespace-nowrap">

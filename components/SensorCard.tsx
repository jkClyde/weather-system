...
export function StatusBadge({ status }: { status: Status }) {
  ...
  return (
    <Badge variant="outline" className={`text-[10px] px-2 py-0.5 ${className}`}>
      {label}
    </Badge>
  );
}
...
// ─── SVG Illustrations ────────────────────────────────────────────────────────
// viewBox="0 0 200 120" — rendered absolutely, full-bleed behind text.
// Sky radial gradients match the card's own Tailwind gradient so there is no
// visible seam. All fills are hardcoded hex — they must NOT invert in dark mode.
...
/**
 * The card's Tailwind gradient background — intentionally matches the
 * illustration's own palette so sky bleeds seamlessly into the card edges.
 */
...
export interface SensorCardProps {
  ...
  value: string;
  unit: string;
  icon: React.ElementType; // kept for API compatibility — not used
  trend: "up" | "down" | "stable";
  trendLabel: string;
  status: Status;
  accentClass: string;   // kept for API compatibility — not used
  iconBgClass: string;   // kept for API compatibility — not used
}
...
export function SensorCard({
  ...
  return (
    <div
      ...
      `}
    >
      {/* Atmospheric SVG illustration — fills the whole card */}
